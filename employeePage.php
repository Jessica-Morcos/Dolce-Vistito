...
<div class="container">
    ...
    // Enable all error reporting for debugging
        ...
        $conn_local = oci_connect('your_username', 'your_password', $local_conn_string);
        if (!$conn_local) {
            $m = oci_error();
            echo "Local Connection Error: " . $m['message'];
        } else {
             // Display success message on successful connection
            echo "Successfully connected with the local database<br>";

             // Handle the 'searchEmployee' form submission
            if (isset($_POST['searchEmployee'])) {
                  // Retrieve and sanitize the employee ID from the form
                $employeeId = $_POST['searchEmployeeId']; 
                echo "Searching for employee with ID: " . $employeeId . "<br/>"; 

               // Prepare SQL query to select employee details
                ...
                if (!$r) {
                    ...
                    if ($row) {
                        ...
                        echo "</table>";
                         ...
                         // Display a message if no employee is found
                        echo "No employee found with ID " . htmlspecialchars($employeeId);
                    }
                }
            }
          
             // Handle the 'addEmployee' form submission
            if (isset($_POST['addEmployee'])) {
                ...
                if (!$resultFinancial) {
                    ...
                    oci_bind_by_name($stmtInsertEmployee, ":totalhours", $totalHours);
            ...
            // Handle the 'deleteEmployee' form submission
            if (isset($_POST['deleteEmployee'])) {
                $employeeId = $_POST['deleteEmployeeId']; // Capture the employee ID
            
            
                 // Delete the specified employee and their financial info
                // Delete employee
                ...
                $resultEmployee = oci_execute($stmtDeleteEmployee, OCI_NO_AUTO_COMMIT);
                 ...
                 // Check and handle the result of the delete operation
                if (!$resultEmployee) {
             ...
             // Handle the 'searchForEdit' form submission
            if (isset($_POST['searchForEdit'])) {
                $employeeId = $_POST['editEmployeeId'];
            
                // Fetch employee and financial information for editing
              ...
              // Handle the 'updateEmployee' form submission
